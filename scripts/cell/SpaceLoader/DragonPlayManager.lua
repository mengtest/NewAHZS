...
function DragonPlayManager:init(SpaceLoader, params)
    log_game_debug("DragonPlayManager:init", "ok")

    local newObj = {}
    newObj.ptr   = {}
    setmetatable(newObj,      {__index = DragonPlayManager})
    ...
    newObj.CellInfo[mission_config.SPECIAL_MAP_INFO_MISSION_ID]          = 31000
...
function DragonPlayManager:Start(SpaceLoader, StartTime)
    ...
    log_game_debug("DragonPlayManager:Start", "missionId=%s;difficult=%s;StartTime=%d",
    ...
    if self.StartTime == 0 then
        ...
        if cfg and cfg['passTime'] > 0 then
            log_game_debug("DragonPlayManager:Start", "passTime=%d", cfg['passTime'])
            local now     = os.time()
            ...
            if endTime > now then
                ...
                log_game_debug("DragonPlayManager:Start", "triggerTime=%d", (endTime - now))
            end
        end
    end
end

function DragonPlayManager:DeathEvent(dbid, SpaceLoader)
	log_game_debug("DragonPlayManager:DeathEvent", "%d", dbid)
...
function DragonPlayManager:ExitMission(dbid)
    log_game_debug("DragonPlayManager:ExitMission", "")
    if self.CellInfo[mission_config.SPECIAL_MAP_INFO_END_TIMER_ID] then
...
function DragonPlayManager:QuitMission(dbid)
    log_game_debug("DragonPlayManager:QuitMission", "")
    if self.CellInfo[mission_config.SPECIAL_MAP_INFO_END_TIMER_ID] then
        local SpaceLoader = self.ptr.theSpaceLoader
        SpaceLoader:delTimer(self.CellInfo[mission_config.SPECIAL_MAP_INFO_END_TIMER_ID])
        if self.dead == -1 then
