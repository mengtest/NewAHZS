...
int CWorldCell::CreateCellInNewSpace(T_VECTOR_OBJECT* p, bool insertAOI)
{
	...
	//更新base/cell共有的属性
		...
		{
			e->SetSpaceID(s->GetId());//kevintest  调用create_avatar_req会调用MapActionMgr:create_cell通过找spaceloader的cell实体来获取对应cspace类实例。否则创建cell在CWorldCell::CreateCellViaMycell会失败return
		}
		//记录base的mailbox
