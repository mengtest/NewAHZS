...
enum
{
	ENUM_LOGIN_CHECK_SUCCESS = 0,  //ENTITY_DEF检查成???
	ENUM_LOGIN_CHECK_ENTITY_DEF_NOMATCH = 1,  //ENTITY_DEF检查不成功
	ENUM_LOGIN_CHECK_NO_SERVICE = 2,  //服务器未开放登???
};

enum
{
	ENUM_LOGIN_SUCCESS = 0,                      //认证成功
	ENUM_LOGIN_RET_ACCOUNT_PASSWD_NOMATCH = 1,   //帐号密码不匹???
	ENUM_LOGIN_NO_SERVICE = 2,                   //服务器未开放登???
	ENUM_LOGIN_FORBIDDEN_LOGIN = 3,              //被禁止登???
	ENUM_LOGIN_TOO_MUCH = 4,                   //服务器人数超过最大数量，不可登录
	ENUM_LOGIN_TIME_ILLEGAL = 5,                 //本次登录超时 
	ENUM_LOGIN_SIGN_ILLEGAL = 6,                 //签名非法
	ENUM_LOGIN_SERVER_BUSY = 7,                  //sdk服务器验证超???
	ENUM_LOGIN_SDK_VERIFY_FAILED = 8,            //sdk服务器验证失???
	ENUM_LOGIN_ACCOUNT_ILLEGAL = 9,              //sdk验证成功但是帐号不一???
};
...
int CWorldLogin::ForbidLogin(T_VECTOR_OBJECT* p)
{
	...
	return ForbidLogin(m_forbiddenLogin, strAccount, endTime);		 //禁账号登???
...
int CWorldLogin::_AccountLoginImpl(T_VECTOR_OBJECT* p, bool bLoginFirst)
{
	...
	//删除账户???密码两边的空???mysql中select * from table where s = '***'中不会区???**是否带空???
	Trim(strAccount);
	Trim(strPasswd);
	...
	if (iter != m_accounts2fd.end())
	{
		...
		else
		{
			...
			{
				//如果两次认证间隔时间小于3???则保留老连???不修改冲突时???;否则踢掉老连???
